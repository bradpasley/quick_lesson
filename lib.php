...
function printHTMLBodyStart(string $pageTitle, int $lessonid=0) {
    ...
    println('<body class="'.DEEPBACKGROUNDSTYLE.'">');
...
function printMenuCard(string $cardTitle, string $cardContent, int $menuScreen=SCREENEXIT, int $lessonid=0, int $moduleid=0, int $conceptid=0) {
    ...
    $buttonName = "Go!";
    $backgroundClass = CARDBACKGROUNDSTYLE;
...
function printJSONModulePage(int $lessonid, int $moduleid) {
    ...
    //println('document.getElementById("conceptID").innerHTML = "JSON Content ID ("+conceptid+")";');
    println('document.getElementById("moduleTitle").innerHTML = moduleJSON["0"].title;');
    //println('document.getElementById("conceptTitle").innerHTML = moduleJSON["1"].title;');
    println('document.getElementById("content").innerHTML = moduleJSON["0"].content;');
...
function printRightArrowButton(string $pageTitle, int $screenType, string $buttonText="", int $lessonid=0, int $moduleid=0, int $conceptid=0) {
    ...
    if($isMainMenuButton || $isLessonButton || $conceptid<$conceptCount) {//don't print right button for last concept.
        ...
        println($middleText);
        println('  <button type="submit" class="btn rounded-pill lh-lg text-light shadow-lg '.LESSONBACKGROUNDSTYLE.'" name="rightArrowButton_'.$pageTitle.'">'.$buttonText.'&nbsp;'.NEXTBUTTONICON.'</button>');
        println('</form>');
...
function printLeftArrowButton(string $pageTitle, int $screenType, string $buttonText="", int $lessonid=0, int $moduleid=0, int $conceptid=0) {
    ...
    if($conceptid>0) {//don't display for first concept
        ...
        println('  <input type="hidden" name="screen" value="'.$screenType.'">');
        println($middleText);
        println('  <button type="submit" class="btn rounded-pill lh-lg bg-primary text-light shadow-lg" name="leftArrowButton_'.$pageTitle.'">'.PREVBUTTONICON.'&nbsp;'.$buttonText.'</button>');
        println('</form>');
...
function printMainMenuButton() {
    ...
    println('  <input type="hidden" name="screen" value="'.SCREENMAINMENU.'">');
    println('<span class="btn rounded-pill lh-lg shadow-lg justify-content-center '.LESSONBACKGROUNDSTYLE.'">');
    println('  <button type="submit" class="btn rounded-pill lh-lg text-light lead '.LESSONBACKGROUNDSTYLE.'" '
